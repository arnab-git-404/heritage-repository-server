...
router.delete('/submissions/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (submission.contentCloudinaryId) {
      await cloudinary.uploader.destroy(submission.contentCloudinaryId).catch(err => 
        console.error('Cloudinary delete error:', err)
      );
    }

    if (submission.translationCloudinaryId) {
      await cloudinary.uploader.destroy(submission.translationCloudinaryId).catch(err => 
        console.error('Cloudinary delete error:', err)
      );
    }

    if (submission.verificationCloudinaryId) {
      await cloudinary.uploader.destroy(submission.verificationCloudinaryId).catch(err => 
        console.error('Cloudinary delete error:', err)
      );
    }
